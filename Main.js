...
let generator = JSON.parse(
  fs
    .readFileSync("./generator.config.json", { encoding: "utf-8" })
    .toString() || ""
);
...
if (!generator == "") {
  ...
  generator = JSON.parse(
    fs.readFileSync("./generator.config.json", { encoding: "utf-8" }).toString()
  );
}
...
class Main {
  ...
  getScriptPath(finalPath, valideFolderName) {
    const scriptExtension = this.extension.mainValidation(
      template,
      "script",
      scriptPossible
    );
    const mainScript = path.join(
      finalPath,
      `${valideFolderName}.${scriptExtension}`
    );
    return mainScript;
  }
  ...
  getTemplate(valideFolderName) {
    ...
    if (!checkTemplate(fileTemplate, componentPossible))
      throw new Error(
        "File Template : class : Class Component || function : Functionnal Component : Specify that in the generator.config file "
      );
    const toModifyTemplate =
      fileTemplate === "function"
        ? fs.readFileSync(__dirname + "/template/rfce.js").toString()
        : fs.readFileSync(__dirname + "/template/rce.js").toString();
    const finalTemplate = toModifyTemplate
      .replaceAll("rfce", valideFolderName)
      .replaceAll("rce", valideFolderName);
    return finalTemplate;
  }

  getStylePath(finalPath, valideFolderName) {
    const styleExtension = this.extension.mainValidation(
      style,
      "style",
      stylePossible
    );
    const stylePath = path.join(
      finalPath,
      `${valideFolderName}.${styleExtension}`
    );
    return stylePath;
  }
